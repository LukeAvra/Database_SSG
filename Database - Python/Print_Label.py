...
def createBarcodeImage(barcodeText, barcode):
    ...
    with PIL.Image.open(os.path.dirname(__file__) + "\\Barcodes\\Base\\4x2.png").convert("RGBA") as base:
        text = PIL.Image.new("RGBA", base.size, (255, 255, 255, 0))
        font = PIL.ImageFont.truetype("C:\\Windows\\Fonts\\Arial.ttf", 20)
    ...
    out = PIL.Image.alpha_composite(base, text)
    
    # open and paste barcode image onto alpha composite image
    barcodeImage = PIL.Image.open(filename)
...
def printBarcode():
    ...
    modelList = [m for m in brother_ql.devicedependent.models]
    ...
    barImage = PIL.Image.open(os.path.dirname(__file__) + "\\Barcodes\\barcode.png")
